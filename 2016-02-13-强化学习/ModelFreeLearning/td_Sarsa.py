...
"""TDSarsa
Temporal Difference Sarsa: 时序差分学习，异策略
"""
...
class TDQ(object):
	...
	# greedy下: 状态 -> 动作
	def action_with_state_for_greedy(self, x_state):
	...
	# normal下: 状态 -> 动作
	def action_with_state_for_normal(self, x_state):
	...
	def updatePi(self):
		...
		for i in range(self.X_count):
			...
			self.Pi_greedy[i, QMaxIndex[i]] = self.Pi_greedy[i, QMaxIndex[i]] + 1.0 - self.greedy

	def walk(self):
		x = random.randint(0, self.X_count-1)	#初始状态随机
		for i in range(2000):
			# 前进一步
			a = self.action_with_state_for_greedy(x)
			x_new = self.state_transfer(x, a)
			r = self.reward(x, a, x_new)
			a_new = self.action_with_state_for_normal(x_new)
			# 更新状态-动作值
			self.Q[x, a] = self.Q[x, a] + self.alpha * (r + self.gamma * self.Q[x_new, a_new] - self.Q[x, a])
			# 更新策略
			self.updatePi()
			# 更新状态
			x = x_new
